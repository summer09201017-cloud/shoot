...
**v26(2026-09-24)第三輪:改成曲線** —— 使用者玩過 v25:「一開頭有點難,炸彈太少,一直殺不完敵人、吃不到武器的寶物;想一開頭簡單一點,但後面才難一些」。
- 所有「隨波數變」的數字集中在 **`WAVE_CURVE`**(常數區 `waveFireMul()` 上方),調曲線只動它:
  - 敵彈冷卻倍率 `fireStart 1.55 → 每波 −0.065 → fireFloor 0.4`(第 19 波到頂)。第 1 波比 v24 還鬆(等效 ENEMY_FIRE_MUL ≈ 3.1),第 5 波 ≈ v24,第 9 波 ≈ v25 開頭,到頂時是第 5 波密度的 2.5 倍。
  - 彈速倍率 `speedStart 0.9 → 每波 +0.016 → speedCap 1.4`(第 1 波 ×0.9×1.12 ≈ 舊版普通;第 32 波到頂)。
  - 掉寶 `waveLootMul()`:前 3 波 ×1.35(武器早到手)、第 12 波起 ×0.85、第 20 波起 ×0.7(滿火力的躺平 bot 靠補血撐到第 30 波,補血要跟著收)。
- 開場炸彈回到原本 10 / 8 / 12;普通掉寶 0.9 → 1。難度預設倍率(v25 那組)、命數、簡單、每日出題不動。
- 量尺(4 種子):普通躺平 177 / 181 / 417 / 420+ 秒(3 顆死,平均第 21 波 —— 開頭活得下來、後面死),硬派躺平 61~351 秒(平均第 10 波),
  彈幕躺平 41~52 秒;閃避 bot 普通/硬派仍活滿(幀級完美,預期)。
- ⚠ **verify-behaviour 的 Q hydra 那項改了量法**:原本「Boss 出場 4.5 秒後畫面上活著的敵彈 > 5」是跟 v23 的 Boss 節奏綁死的快照
  (v25 起 Boss 節奏跟難度 bulletRate 走,普通快 30%,快照那一刻剛好只剩一組扇形彈 4~5 顆 ⇒ 假紅);改成 4.5 秒內**累計**發出的敵彈數。
  改 Boss 節奏或難度倍率時,若這項又紅,先想「是節奏變了還是 Boss 壞了」。

**v27(2026-09-24 晚)第四輪:再難一些 + 手機下方按鈕縮小** —— 使用者玩到第 7 波(火力 20/20、剩 2 命 11 炸)截圖:「下方的 3 顆按鈕太大、太高了,難度請再難一些」。
- `WAVE_CURVE` 整體抬高、爬得更陡:冷卻 `1.45 → −0.085/波 → 0.35`(第 14 波到頂 = 第 1 波密度 4.1 倍;開頭仍比 v24 的等效 1.3 鬆),
  彈速 `0.9 → +0.022/波 → 1.5`。新增兩條隨波:**小兵血量** `waveHpMul()` 第 8 波起 +3%/波、封頂 ×1.6(火力 20 後期不再秒殺一切,敵人活久一點才會開槍);
  **Boss 出招間隔** `bossWaveMul()` 第 10 波起 −2%/波、封頂 ×0.7(乘在 runBossPattern 的 patternTimer 上)。兩個都不抽亂數。
- **量尺要 10 顆種子才看得出趨勢**:4 顆時「普通躺平」在更難的 v27 反而比 v26 活更久(掉寶 RNG 鏈一動,單顆種子命運翻盤)。
  改用 `MD_DIFFS=normal MD_BOTS=idle,dodge MD_SEEDS=10` 兩版各跑一次 A/B:躺平平均 **第 16.1 波 → 第 12.3 波**、活滿 420 秒 2 顆 → 1 顆、中位數 184 秒 → 92 秒;
  閃避被打 10 → 12 發。以後判斷「有沒有變難」只認這種 A/B,不認單次 4 顆。
- **手機直向按鈕**(styles.css 檔尾 `@media (max-width: 680px) and (orientation: portrait)`):BOMB 80→60、FOCUS 64→52、SKILL 56→48、暫停 52→44,全部仍 ≥44px。
